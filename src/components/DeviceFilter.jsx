...
export default function DeviceFilter(props) {
  ...
  const handleFieldChange = (event) => {
    switch (event.target.value) {
      ...
      case `IntendedAnatomy`: {
        let tmp = []
        if (props.filter.IntendedAnatomy !== undefined) {
          tmp = [...props.filter.IntendedAnatomy]
        }
        handleIntendedAnatomyChange(null, tmp, event.target.value)
        break
      }
  ...
  const handleIntendedAnatomyChange = (event, anatomy, field) => {
    let data = []
    ...
    <FormGroup row>
      <FormControl className={classes.formControl}>
        ...
        >
          ...
          <MenuItem value={`IntendedAnatomy`}>Intended Anatomy</MenuItem>
      ...
      {props.filter.field === `IntendedAnatomy` && (
        <FormControl className={classes.formControl}>
          <InputLabel>Intended Anatomy</InputLabel>
          <Select
            multiple
            value={props.filter.IntendedAnatomy}
            input={<Input />}
            renderValue={(selected) => selected.join(`, `)}
            onChange={handleIntendedAnatomyChange}
            MenuProps={MenuProps}
            inputProps={{ readOnly: props.filter.lock }}
          >
            {props.filterData.Anatomy !== undefined &&
              props.filterData.Anatomy.map((a, i) => (
                <MenuItem key={i} value={a}>
                  <Checkbox
                    checked={props.filter.IntendedAnatomy.includes(a)}
                  />
                  <ListItemText primary={a} />
                </MenuItem>
              ))}
